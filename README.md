...
1. **Clone the repository**:
   ```bash
   git clone https://github.com/your-repo/platform-engineer-assignment.git
...
3. **Set up GCS bucket for Terraform state**:
   Create a Google Cloud Storage bucket and update the bucket name in the `TFSTATE_BUCKET` environment variable in `.github/workflows/ci-cd.yml`.

### Running the Pipeline (example on the `main` branch but same for `dev`)

1. **Push changes to the `main` branch** for production deployment:
   ```bash
   git add .
